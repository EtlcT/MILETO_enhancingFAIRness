...
class Model:
    ...
    def process_meta_dc_terms(self, values):
        """Update meta_dc_terms table based on user input"""
    ...
    def load_spreadsheet(self, filepath):
        ...
        try:
            self.tmp_data = pd.read_excel(
                filepath, sheet_name=None, keep_default_na=False)
        except Exception as e:
            logging.error(
                f"An error occured reading your file but it si probably not due to the app {str(e)}")
        else:
